...
// MODIFICAT: Construiește prompt dinamic pentru întrebări cu grile cu configurare flexibilă
...
// MODIFICAT: Construiește prompt dinamic pentru răspunsuri scurte cu configurare flexibilă
function buildShortPrompt(stepData, student, answer, exerciseConfig) {
  if (!exerciseConfig.has_scoring) {
    // Pentru exerciții fără punctaje - doar feedback educativ
...
- Oferă doar feedback educativ, fără punctaj
...
RĂSPUNDE DOAR CU TEXTUL FEEDBACK-ULUI, FĂRĂ JSON SAU ALTE FORMATĂRI.`;
    ...
    // Pentru exerciții cu punctaje - feedback cu scoring
    return `Ești un profesor de TIC specializat în securitate digitală pentru clasa XII. Evaluează răspunsul și oferă feedback în 3-4 propoziții cu ton încurajator și educativ.
...
// MODIFICAT: Construiește prompt pentru raportul final cu configurare flexibilă
...
RĂSPUNDE DOAR CU TEXTUL RAPORTULUI FINAL, FĂRĂ JSON SAU ALTE FORMATĂRI.`;
    ...
    // Pentru exerciții cu punctaje - logica existentă
...
async function handleStepFeedback(requestData) {
  ...
  try {
    ...
    if (stepData.type === 'grila') {
      ...
      // Calculează scorul doar pentru exerciții cu punctaje
      if (exerciseConfig.has_scoring) {
      ...
      if (exerciseConfig.has_scoring) {
        // Pentru exerciții cu punctaje - extrage scorul
        ...
        // Pentru exerciții fără punctaje - tot textul este feedback
        feedback = aiText;
        score = 0;
      }
    } else {
    ...
    // MODIFICAT: Returnează datele adaptate pentru configurația exercițiului
    const responseData = {
      success: true,
      feedback: feedback,
      aiGenerated: true,
    ...
    // Adaugă datele de scoring doar pentru exerciții cu punctaje
    if (exerciseConfig.has_scoring) {
      responseData.score = score;
      responseData.maxPoints = stepData.points;
    } else {
      responseData.score = 0; // Pentru compatibilitate cu submit-step.js
    }
...
// MODIFICAT: Gestionează raportul final cu configurare flexibilă
async function handleFinalReport(requestData) {
  ...
  // Pentru exercițiul de securitate digitală, forțează has_scoring = true pentru compatibilitate
  const finalExerciseConfig = exerciseConfig || { has_scoring: true };
  ...
  try {
    ...
    console.log('Generez raport AI final pentru:', {
      ...
      hasScoring: finalExerciseConfig.has_scoring,
    });
