// worksheets/tic/XII/hardware/worksheet.js
// Logica specifică pentru testul "Hardware & Performanță"
...
function initializeSpecificWorksheet(authData) {
  ...
  if (worksheetSteps.length === 0) {
    showMessage('Nu există întrebări definite pentru acest test', 'error');
    return;
  }
...
// Creează o întrebare folosind template-urile HTML
...
// Configurează o întrebare cu grilă
function setupGrilaStep(stepElement, stepData, stepIndex) {
  ...
  if (!stepData.options || stepData.options.length === 0) {
    console.error(`Opțiunile lipsesc pentru întrebarea ${stepIndex + 1}`);
    return;
  }
...
// Configurează o întrebare cu răspuns scurt
function setupShortStep(stepElement, stepData, stepIndex) {
  ...
  function showPasteAlarm(e, revertValue = null) {
    ...
    textarea.value =
      '🤖 AI detected: Te-am prins! Nu copia răspunsurile. Vrei să înveți sau doar să copiezi? Pentru BAC trebuie să știi, nu să copiezi! 💻';
...
// Funcția pentru trimiterea întrebării curente
...
// Finalizează testul cu raport AI
async function finalizeWorksheet() {
  const allCompleted = Object.values(studentProgress).every((p) => p && p.completed);
  if (!allCompleted) {
    showMessage('Toate întrebările trebuie completate înainte de finalizare', 'warning');
    return;
  }
  ...
  try {
    ...
    showMessage('Test finalizat cu raport AI complet!', 'success');
  } catch (error) {
    ...
    displayCompletionWithAIReport(
      totalScore,
      maxScore,
      'Testul a fost finalizat cu succes. Raportul AI detaliat nu este disponibil momentan, dar toate răspunsurile au fost evaluate individual.',
      exerciseConfig
    );
    showMessage('Test finalizat fără raport AI detaliat', 'warning');
  } finally {
...
async function requestFinalAIReport(totalScore, maxScore, exerciseConfig) {
  ...
  try {
    const response = await fetch('/.netlify/functions/worksheet-submit-tic-XII-hardware', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function setFinalizationUIState(isLoading) {
  ...
  if (isLoading) {
    ...
    showMessage('AI analizează cunoștințele despre hardware...', 'info');
  } else {
...
function displayCompletionWithAIReport(totalScore, maxScore, aiReport, exerciseConfig) {
  ...
  if (exerciseConfig.has_scoring) {
    ...
    scoreElement.innerHTML = `
      <div class="completion-display">
        <span class="completion-value">Test completat cu succes!</span>
  ...
  feedbackElement.innerHTML = `
    <div class="ai-final-report">
      <h4>📊 Raport final - Hardware & Performanță</h4>
...
function addCompletionActionButtons() {
  ...
  if (canStartNew) {
    ...
    retryButton.textContent = 'Refă testul';
  ...
  if (!canStartNew) {
    ...
    infoMessage.innerHTML = `
      ...
      <p>Poți vedea răspunsurile tale sau să te întorci la pagina principală pentru alte teste.</p>
    `;
    actionsContainer.insertBefore(infoMessage, actionsContainer.firstChild);
  }
}

// Gestionează reluarea testului
async function handleRetryExercise() {
  showConfirmModal(
    'Refaci testul?',
    'Sigur vrei să refaci testul? Vei începe de la zero și vei pierde progresul actual.',
    async () => {
      const retryBtn = document.querySelector('.retry-btn');
      const originalText = retryBtn.textContent;
      retryBtn.disabled = true;
      retryBtn.textContent = 'Se pregătește testul...';
      ...
      try {
        ...
        showMessage('Test resetat cu succes! Poți începe din nou.', 'success');
      } catch (error) {
        console.error('Eroare la reluarea testului:', error);
        showMessage('Eroare la reluarea testului. Încearcă din nou.', 'error');
...
function handleGoHome() {
  showConfirmModal(
    'Părăsești testul?',
    'Sigur vrei să părăsești acest test și să te întorci la pagina principală?',
    () => {
